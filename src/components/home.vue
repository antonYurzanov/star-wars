<template lang='pug'>
    .star-wars
        vue-headful(title="Starships")
        div.preloader(v-if="preloader") Идет загрузка...
        template(v-else)
            form.search
                input.search-text(v-model="filter" name="ship" value="" placeholder="Search ship...")
            ul.ships
                template(v-if="filter")
                    li.ship(v-for="ship of searchIcons")
                        img.ship-icon(src="https://img.icons8.com/wired/48/000000/space-ship.png" alt="")
                        span.ship-name {{ ship.name }}
                        button.ship.full(@click.prevent="getShip(ship.url)") Подробнее
                template(v-else)
                    li.ship(v-for="ship of ships")
                        img.ship-icon(src="https://img.icons8.com/wired/48/000000/space-ship.png" alt="")
                        span.ship-name {{ ship.name }}
                        button.ship.full(@click.prevent="getShip(ship.url)") Подробнее
            .pages
                button(@click.prevent="prevPage" v-if="previous !== null") Prev
                button(@click.prevent="nextPage" v-if="next !== null") Next
</template>
<script>
    export default {
        name: 'StarWars',
        ...
        methods: {
            ...
            nextPage() {
                this.preloader = true;
                let page = this.next.replace('https://swapi.co/api/starships/?page=', '');
                if (this.$route.query.filter) {
            ...
            prevPage() {
                this.preloader = true;
                let page = this.previous.replace('https://swapi.co/api/starships/?page=', '');
                if (this.$route.query.filter) {
                    window.history.pushState(null, null, window.location.search.split('?')[0] + '?page=' + page + '&filter=' + this.$route.query.filter);
                } else {
                    window.history.pushState(null, null, window.location.search.split('?')[0] + '?page=' + page);
